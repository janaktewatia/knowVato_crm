...
export default function ActionColumn({ lead, onMessage, onEdit, onService, onFollowUp, onJourney, onRegistration }) {
  const { can } = useAuth();
  ...
  if (can("leads", "edit")) {
    actions.push({ icon: "clipboard-check", label: "Registration", onClick: onRegistration, color: "#1f5f8b" });
  }
  if (can("leads", "view")) {
